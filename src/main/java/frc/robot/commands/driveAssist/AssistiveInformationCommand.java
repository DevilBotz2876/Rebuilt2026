...
public class AssistiveInformationCommand extends Command {
  ...
  public void initialize() {
        ...
        if(alliance.isPresent()) {
            if(alliance.get() == Alliance.Red) {
                HUB_POSE = FlippingUtil.flipFieldPose(new Pose2d(DynamicLocation.HUB.getX(), DynamicLocation.HUB.getY(), Rotation2d.kZero));
                return;
            }
        }
  ...
  @Override
  public void execute() {
    SmartDashboard.putNumber("Controls/Needed Rotation", 
        Units.radiansToDegrees(drive.getPose().getRotation().getRadians() - 
    ...
    double distanceFromLocation = HUB_POSE.getTranslation().getDistance(drive.getPose().getTranslation());
    ...
    // }
    // correctionNeed = Math.abs(distanceAwayCorrectDistance) <= validDistanceTolerance ? 0 : (int) Math.signum(distanceAwayCorrectDistance);
