...
public enum DynamicLocation {
    ...
    public boolean equal(DynamicLocation location) {
        return this.x == location.getX() && this.y == location.getY() && this.name == location.getName();
    }

    public static Command createPathfindingToLocationCommand(DynamicLocation location, Rotation2d endRot, PathConstraints constraints) {
        return createPathfindingToLocationCommand(new Pose2d(location.x, location.y, endRot), constraints, 0.0);
    }

    public static Command createPathfindingToLocationCommand(DynamicLocation location, Rotation2d endRot, PathConstraints constraints, double endVelocity) {
        return createPathfindingToLocationCommand(new Pose2d(location.x, location.y, endRot), constraints, endVelocity);
    }

    public static Command createPathfindingToLocationCommand(Pose2d target, PathConstraints constraints, double endVelocity) {
